...
public abstract class GenericController<TEntidade extends GenericEntity, TDto> {
	...
	@GetMapping
	public ResponseEntity<Page<List<TEntidade>>> pesquisar(@PageableDefault(size = 5, sort = "name", direction = Direction.ASC) Pageable pageable, @RequestParam String name){
	...
	@GetMapping("/{uuid}")
	public ResponseEntity<TEntidade> picaresquesPortAudio(@PathVariable UUID uuid) {
	...
	protected PlaylistEntity convertDTO2Entity1() {
